...
public class KeyMapping implements Comparable<KeyMapping> {
   ...
   private static final Map<String, Integer> CATEGORY_SORT_ORDER = Util.make(Maps.newHashMap(), (categories) -> {
      categories.put("key.categories.movement", 1);
      categories.put("key.categories.gameplay", 2);
      categories.put("key.categories.inventory", 3);
      categories.put("key.categories.creative", 4);
      categories.put("key.categories.multiplayer", 5);
      categories.put("key.categories.ui", 6);
      categories.put("key.categories.misc", 7);
      categories.put("key.categories.pixlclient", 8);
   });
   ...
   public KeyMapping(String name, int p_90822_, String category) {
      this(name, InputConstants.Type.KEYSYM, p_90822_, category);
   }
