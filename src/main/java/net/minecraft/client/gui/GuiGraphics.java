...
public class GuiGraphics {
   ...
   public int drawString(Font font, @Nullable String text, int x, int y, int color, boolean shadow) {
      if (text == null) {
         return 0;
      } else {
   ...
   public int drawString(Font font, FormattedCharSequence text, int x, int y, int color, boolean shadow) {
      int i = font.drawInBatch(text, (float)x, (float)y, color, shadow, this.pose.last().pose(), this.bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
      this.flushIfUnmanaged();
      return i;
   }
   ...
   public void drawWordWrap(Font font, FormattedText text, int x, int y, int color, int p_282629_) {
      for(FormattedCharSequence formattedcharsequence : font.split(text, color)) {
         this.drawString(font, formattedcharsequence, x, y, p_282629_, false);
         y += 9;
      }
   ...
   public void blitRepeating(ResourceLocation p_283059_, int p_283575_, int p_283192_, int p_281790_, int p_283642_, int p_282691_, int p_281912_, int p_281728_, int p_282324_) {
      ...
      for(IntIterator intiterator = slices(p_281790_, p_281728_); intiterator.hasNext(); i += j) {
         ...
         for(IntIterator intiterator1 = slices(p_283642_, p_282324_); intiterator1.hasNext(); l += i1) {
            ...
            this.blit(p_283059_, i, l, p_282691_ + k, p_281912_ + j1, j, i1);
         }
      }
   }
