...
public class PlayerInfo {
   ...
   // PIXLCLIENT START
   public boolean hasTextureLocation(MinecraftProfileTexture.Type type) {
      return textureLocations.containsKey(type);
   }

   public ResourceLocation getTextureLocation(MinecraftProfileTexture.Type type) {
      return textureLocations.get(type);
   }

   public void setTextureLocation(MinecraftProfileTexture.Type type, ResourceLocation resourceLocation) {
   ...
   protected void registerTextures() {
      this.registerTextures(false);
   }

   protected void registerTextures(/* PIXLCLIENT START */ final boolean force /* PIXLCLIENT STOP */) {
      synchronized(this) {
         if (!this.pendingTextures) {
            this.pendingTextures = true;
            Minecraft.getInstance().getSkinManager().registerSkins(this.profile, (type, resourceLocation, profileTexture) -> {
               /* PIXLCLIENT START */
               if (type.equals(Type.CAPE) && this.textureLocations.containsKey(Type.CAPE) && this.textureLocations.get(Type.CAPE) != null && !force)
                  return;
               /* PIXLCLIENT STOP */
               this.textureLocations.put(type, resourceLocation);
               if (type == Type.SKIN) {
                  this.skinModel = profileTexture.getMetadata("model");
